...
def redraw_window(win, board, time, strikes, button):
    ...
    label = pygame.Surface((90, 45))
    label.fill((255, 255, 255))
    text = fnt.render('Solve', 1, (0, 0, 0))
    label.blit(text, (10, 10))
    win.blit(label, (10, 550))
    # Draw grid and board
    board.draw(win)
...
def main():
    ...
    board = Grid(bo, 9, 9, 540, 540)
    key = None
    run = True
    start = time.time()
    strikes = 0
    button = pygame.Rect(20, 546, 100, 50)
    while run:
